...
async function performPaste(targetFolderId = null) {
    ...
    // "Confirm File Migration?" — required by the system architecture
    // before any copy/cut paste is committed to the new location.
    const verb  = clip.mode === 'copy' ? 'copy' : 'move';
    const where = targetFolderId ? 'into the selected folder' : 'into the current folder';
    if (!confirm(`Confirm file migration: ${verb} ${clip.items.length} item(s) ${where}?`)) {
        showToast("Paste cancelled.", "error");
        return;
    }
...
async function renderVaultFiles() {
    ...
    try {
        ...
        vaultState.items = [
            ...((data && data.folders) || []).map(f => ({ ...f, type: 'folder' })),
            ...((data && data.files)   || []).map(f => ({ ...f, type: 'file' }))
        ];
...
function previewFile(item) {
    ...
    document.getElementById("prevPath").innerText = currentPath.length > 0 ? currentPath.map(p => p.name || 'Unknown').join(' / ') : 'Departments Overview';
